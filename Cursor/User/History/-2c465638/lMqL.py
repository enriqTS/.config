...
def process_post_template_steps_batch(api_url: str, jwt_token: str, telefones: list,
                                      ...
                                      session_attributes_por_telefone: dict,
                                      veiculo_data_por_telefone: dict = None) -> Dict[str, Any]:
    """
    Processa etapas pos-template para batch de motoristas

    Input: api_url (str), jwt_token (str), telefones (list), chatbot_lambda_name (str), template_messages (dict), motoristas_info (dict), session_attributes_por_telefone (dict), veiculo_data_por_telefone (dict opcional)
    ...
    for telefone in telefones:
        ...
        veiculo_data = veiculo_data_por_telefone.get(telefone, {}) if veiculo_data_por_telefone else {}
...
def get_veiculo_id_by_name(nome_veiculo: str) -> int:
    """
    Mapeia nome de tipo de veiculo para seu ID

    Input: nome_veiculo (str)
    Output: (int) - ID do tipo de veiculo ou None se nao encontrado
    """
    veiculo_map = {
        "VUC": 1,
        "Caminhão Toco": 2,
        ...
        "Carreta 3 Eixos": 10,
        "Carreta Canguru": 11,
        "Carreta Vanderleia": 12,
        "Carreta Bi-Trem (7 eixo)": 13,
        "Carreta Bi-Trem (9 eixo)": 14,
        "Carreta Rodo-Trem": 15,
        "Carreta Treminhão": 16,
        "Carreta 4 Eixos": 19,
        "Carro": 20
    }
    return veiculo_map.get(nome_veiculo)

def get_equipamento_id_by_name(nome_equipamento: str) -> int:
    """
    Mapeia nome de tipo de equipamento para seu ID

    Input: nome_equipamento (str)
    Output: (int) - ID do tipo de equipamento ou None se nao encontrado
    """
    equipamento_map = {
        "Basculante": 1,
        "Baú Frigorificado": 2,
    ...
    return equipamento_map.get(nome_equipamento)

def build_chatbot_context_message_dual(template_message: str, cadastrado_telefone: bool,
                                       cadastrado_cpf: bool, session_attrs: dict = None,
                                       veiculo_data: dict = None) -> str:
    """
    Monta a mensagem de contexto para o chatbot, incluindo template enviado, status de cadastro, session attributes e veiculos permitidos.
    Inputs: template_message (str), cadastrado_telefone (bool), cadastrado_cpf (bool), session_attrs (dict opcional), veiculo_data (dict opcional)
    ...
    # Add vehicle types and equipment if available
    if veiculo_data:
        tipos_veiculo = veiculo_data.get('tipos', [])
        if tipos_veiculo:
            tipo_ids = []
            for tipo_nome in tipos_veiculo:
                tipo_id = get_veiculo_id_by_name(tipo_nome)
                if tipo_id:
                    tipo_ids.append(str(tipo_id))
            if tipo_ids:
                session_info_lines.append(f"- Tipos de veiculo permitidos (IDs): {', '.join(tipo_ids)}")

        equipamentos = veiculo_data.get('equipamentos', [])
        if equipamentos:
            equip_ids = []
            for equip_nome in equipamentos:
                equip_id = get_equipamento_id_by_name(equip_nome)
                if equip_id:
                    equip_ids.append(str(equip_id))
            if equip_ids:
                session_info_lines.append(f"- Equipamentos permitidos (IDs): {', '.join(equip_ids)}")
...
def lambda_handler(event, context):
    ...
    for motorista in motoristas:
        ...
        if telefone_raw:
            ...
            # Extrair dados de veiculo da oferta
            if current_oferta_data.get('veiculo'):
                veiculo_data_por_telefone[telefone] = current_oferta_data['veiculo']
                logger.info(f"[INFO] Adicionado veiculo data para {telefone}: tipos={len(current_oferta_data['veiculo'].get('tipos', []))}, equipamentos={len(current_oferta_data['veiculo'].get('equipamentos', []))}")
